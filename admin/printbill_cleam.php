...
<title>  พิมพ์ใบสัญญาเคลมสินค้าเปลี่ยนคืน  </title>
</head>  
<style>
@font-face {
...
?>
  ...
  <style>
  	...
  	<?php
		...
		$menu_id_back = $_GET["product_bk"];
		$menu_id_new = $_GET["product_new"]; 
	?> 
	...
	?>
      ...
      <?php
		...
		$p_dataname = "";
		$codeno = ""; 
		$sql = "SELECT * FROM product where pk = '".$menu_id_new."'   order by pk asc  "; 
		$query = mysqli_query($objCon,$sql);
		while($objResult = mysqli_fetch_array($query))
		{  
			$p_data1 = $objResult["appleid"];
