...
include('../database.php');
		...
		if(!empty($_SESSION["UserID"])){
			...
			$strSQL = " Update list_order Set status =  '".$status."', closebill = 'หมดหนี้'  " ;
			$strSQL .=" WHERE bill_no = '".$bill_no."' "; 
			$objQuery = mysqli_query($objCon, $strSQL);

			$strSQL = " Update history_payment Set status =  '".$status."', closebill = 'หมดหนี้'  " ;
			$strSQL .=" WHERE bill_no = '".$bill_no."' "; 
			$objQuery = mysqli_query($objCon, $strSQL);
			...
			$strSQL = " Update list_order Set status =  '".$status."'  " ;
			$strSQL .=" WHERE bill_no = '".$bill_no."' "; 
			$objQuery = mysqli_query($objCon, $strSQL);

			$strSQL = " Update history_payment Set status =  '".$status."'  " ;
			$strSQL .=" WHERE bill_no = '".$bill_no."' "; 
			$objQuery = mysqli_query($objCon, $strSQL);
			}
