...
include("../database.php");
    ...
    $i = 1;
    $sql = "SELECT * FROM product where typedata = '".$typedata."' and total = '1'  order by pk asc "; 
    ...
    { 
		...
		$showclose6 = "-";
		$sql_chk = " SELECT * FROM drop_typestore where pk = '".$objResult["storerage"]."' order by pk asc limit 1 ";   
		$query_chk = mysqli_query($objCon,$sql_chk); 
		while($objResult_chk = mysqli_fetch_array($query_chk))
		{
         ...
         $html .= '<option value="'.$objResult["pk"]."///".$objResult["name"]."///".$showclose."///".$showclose6."///".$showclose2."///".$objResult["price2"]."///".$showclose3."///".$showclose4."///".$objResult["price1"]."///".$objResult["codeno"]."///".$showclose5.'"> '.$objResult["name"].' </option>';
