...
include('../database.php');
	...
	if(empty($create_by)){
		...
		$bill_no = ""; 
		$GGyear= date('Y')+543 ; 
		$sql2 = "SELECT count(pk) as total FROM run_bill_product_pasy  ";
		...
		$bill_no =  "SGBNUC".date('Ymd')."-".$total; 
		$strSQL = "INSERT INTO run_bill_product_pasy (bill_no) VALUES  ('".$bill_no."')";  
		...
		$objQuery = mysqli_query($objCon, $strSQL); 
		 ...
		 echo("<SCRIPT LANGUAGE='JavaScript'> window.open('print_product_pasy.php?pasy_bill=".$bill_no."'); </script>"); 
...
<form id="frm" method="post" action="product_pasy.php?page=2">
