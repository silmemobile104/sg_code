...
include('../database.php');
	...
	$searchname = date('d-m-Y');
	if(!empty($_GET["startdate"])){
		$searchname = $_GET["startdate"]; 
		$daystart_load = date("Y-m-d", strtotime($searchname)); 
	}else{ 
		$daystart_load = date("Y-m-d", strtotime($searchname)); 
		
	}
	$searchname2 = date('d-m-Y');
	if(!empty($_GET["endate"])){
		$searchname2 = $_GET["endate"]; 
		$daystart_load2 = date("Y-m-d", strtotime($searchname2)); 
...
<th width="4%" bgcolor="#BEC6CB" style="border: 0px solid #FFF;  border-right: 1px solid #FFF;  "><div align="center"> 
<font size="3px" class="serif2" color="#000" style=" font-size: 13px; ">  COD   </font></div></th>    
<th width="4%" bgcolor="#BEC6CB" style="border: 0px solid #FFF;  border-right: 1px solid #FFF;  "><div align="center"> 
...
<font size="3px" class="serif2" color="#000" style=" font-size: 13px; ">  ยอดเงิน   </font></div></th>  
	    ...
	    <tbody>
										...
										$sql2 = " SELECT a.*, b.name, c.typedata_2 FROM list_order  a
											...
											where a.bill_no != ''     and a.chk = '' and a.status = 'ปกติ' and c.computer != 'ชำระค่าคอมไปแล้ว' and a.contactstatus = 'อนุมัติแล้ว'
