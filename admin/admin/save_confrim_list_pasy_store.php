...
include('../database.php');
	...
	if(empty($create_by)){
		...
		$bill_no = ""; 
		$GGyear= date('Y')+543 ; 
		$sql2 = "SELECT count(pk) as total FROM run_bill_pasy2  ";
		...
		$bill_no =  "BSGTDC".date('Ymd')."-".$total; 
		$strSQL = "INSERT INTO run_bill_pasy2 (bill_no) VALUES  ('".$bill_no."')";  
		...
		$objQuery = mysqli_query($objCon, $strSQL); 
		 ...
		 echo("<SCRIPT LANGUAGE='JavaScript'> window.open('print_list_pasy_store.php?pasy_bill=".$bill_no."'); </script>"); 
...
<form id="frm" method="post" action="list_pasy_store.php?page=2">
