...
include('header.php');
	...
	{  
		...
		$newcalculator = $price2 - $price1;
		$totalprice = 0;
		if($newcalculator <= 0){
			$totalprice = 0; 
		}else{ 
			$totalprice = $newcalculator; 
		}
                     ...
                     <font size="4px" color="#000000">  เพิ่มรายการสินค้า/ราคาสินค้า/สต๊อก   </font>   
                      ...
                      <?php if(empty($_GET["page"])){ ?>
							...
							<div class="form-group"> 
							    <a data-toggle="modal" data-target="#smallmodaltwocheck"  class=" btn btn-sm " style="background-color: #F77369; border-radius: 5px;  border-color: #F77369; border: 1px solid  #F77369;   margin-top: -5px;   " > 
								...
								<!-- modal small -->
								<div class="modal fade" id="smallmodaltwocheck" tabindex="-1" role="dialog" aria-labelledby="smallmodalLabel" aria-hidden="true">
								<div class="modal-dialog modal-md" role="document">
											...
											<?php
											$ic = 1;
											$sql_v = "SELECT * FROM update_real_time where bill_no = '".$bill_no."'    order by pk asc  ";   
											...
											while($objResult_v = mysqli_fetch_array($query_v))
											{
													...
													echo $ic.". ".$memberlast1 . " " . $objResult_v["status"] .
														" <br> <font color = 'red' > ทำรายการ : ". DateThai($memberlast2)." ".Datethai2($memberlast2) . " </font> 
														
														<br> <font color = 'red' > เวลา : ".  $memberlast3  . " </font>   <Br> 
														-------------------------- <Br> ";
                        ...
                        <?php } ?>
							...
							<div class="form-group">
							   <font color = '#000' size="3px" > สถานะสินค้า </font>      
							    <div class="input-group   "  style="border: 1px solid #C9C9C9; border-radius: 4px; height: 35px;   ">  
							    <select class="form-control" id="comeback" name="comeback"    style=" width:30px; -webkit-appearance: none; border:  0px solid #FFF; border-radius: 5px;   height: 30px; font-size: 15px; "  >
							   
							    		<?php 
										$sql = "SELECT * FROM drop_comeback  where name = '".$comeback."'  order by pk asc  "; 
										$query = mysqli_query($objCon,$sql);
										while($objResult = mysqli_fetch_array($query))
										{ 
										?>
                            ...
                            <div class="col-md-4"  >
							<div class="form-group">
							   <font color = '#000' size="3px" > สถานะการขาย </font>      
							    <div class="input-group   "  style="border: 1px solid #C9C9C9; border-radius: 4px; height: 35px;   ">  
							    <select class="form-control" id="drop_buy" name="drop_buy"    style=" width:30px; -webkit-appearance: none; border:  0px solid #FFF; border-radius: 5px;   height: 30px; font-size: 15px; "  >
							   
							    		<?php 
										$sql = "SELECT * FROM drop_buy  where name = '".$drop_buy."'  order by pk asc  "; 
										$query = mysqli_query($objCon,$sql);
										while($objResult = mysqli_fetch_array($query))
										{ 
										?>
