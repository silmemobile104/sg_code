...
?>
  ...
  <style>
	...
	*/
	...
	$sql2 = " SELECT a.*, b.name, b.facebook, b.telphone, b.passport 
	 FROM money_customer_bank  a Inner Join customer b On   a.customer = b.pk 
	 where a.bill_no != '' and a.bill_no_ref = '".$bill_no."'  order by a.pk   ";   
	...
	{      
		...
		$price = $objResult2["money"];	 								 
		$pkcheck  = $objResult2["pk"];								 
		$bill_no_old  = $objResult2["bill_no"];								 

	}
	
	$img = "";
	$sql_c2 = "SELECT * FROM product_img where bill_no = '".$bill_no."'    order by pk asc limit 1  "; 
	$query_c2 = mysqli_query($objCon,$sql_c2);
	while($objResult_c2 = mysqli_fetch_array($query_c2))
	{  
		$img =  $objResult_c2["img"]; 
	}
	...
	$get_money = 0;
	$sql = "SELECT * FROM money_customer_bank where bill_no_ref = '".$bill_no."'   order by pk desc   ";  
	 
	$query = mysqli_query($objCon,$sql); 
	while($objResult = mysqli_fetch_array($query))
	{ 
		$get_money = $objResult["money"];
	} 
	
	
	$total_p = 0;
	$sql = "SELECT * FROM money_customer_bank where bill_no = '".$bill_no_old."'  and pk  <= '".$pkcheck."'
	order by pk desc   ";  
	 
	$query = mysqli_query($objCon,$sql); 
	while($objResult = mysqli_fetch_array($query))
	{ 
		if($objResult["typegetpayment"] == "รับเงินสด"){
			$total_p += $objResult["money"]; 
		}else if($objResult["typegetpayment"] == "โอนผ่านบัญชีบริษัท"){
			$total_p += $objResult["money"]; 
		}
		  
		if($objResult["typegetpayment"] == "ชำระหักเงินฝาก"){
			$total_p -= $objResult["money"]; 
		}else if($objResult["typegetpayment"] == "ชำระ2ทาง"){
			$total_p -= $objResult["money"]; 
		}
	} 
...
?>
  ...
  <table width="100%" style="border: 1px solid #000; ">
  	<tr>
			 ...
			 <td align="left" width="60%" valign="top"  >   
				<font size="2px" style="font-size: 18px;">   
			 	 ...
			 	 <b> &nbsp; ข้อมูลผู้ฝากเงิน  </b>  <?php echo $namemember1;  ?>  โทร.  <?php echo $namemember2;  ?>  
		 	 	 <div style="margin-top: 5px; ">  </div>
			 	 <b> &nbsp; ยอดเงินฝาก   </b>   <?php echo number_format(0+$get_money); ?> บาท   
		 	 	 <div style="margin-top: 5px; ">  </div>
			 	 <b> &nbsp; รวมยอดเงินฝากสุทธิ์   </b>   <?php echo number_format(0+$total_p); ?> บาท   
		 	 	 <div style="margin-top: 5px; ">  </div>
				</font> 
			 </td> 
				...
				<font size="2px" style="font-size: 18px;">   
		 	 	 <div style="margin-top: 5px; ">  </div>
					   ...
					   <?php if(!empty($img)){ ?>
							...
							<img src="../img/<?php echo $img; ?>" style=" width: 70%;  ">  
    	...
    	<br>  .................................................................................................................... </font>
			<br> 
				<font size="2px" color="Black" style="margin-left: -20px; font-size: 18px; "> ผู้นำฝาก  </font> 
    	<br>  
    	<br>  
    	<br>  
    	<br>   
