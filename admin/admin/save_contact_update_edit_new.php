...
include('../database.php');
	...
	{	
		...
		$strSQL = " Delete From history_payment  ";
		...
		$strSQL = " Delete From list_order_cleam  ";
		$strSQL .=" WHERE bill_no = '".$bill_no."' ";
		$objQuery = mysqli_query($objCon,$strSQL); 	
		...
		percent, computer, major, closebill, onoff, computer2, priceorder, appleid, password, onoff_copy, bank, pasycal, qrdata, contactstatus, daypayment

		) VALUES 
		(
		'".$bill_no."' ,'' ,'".$menu_id."',
		'".$money."', '".$daytotal."', '".$dayprice."', '".$daytotal."', '".$daystart."', '".$endate."', '1', 
		'".$create_date."', '".$create_date2."', '".$_SESSION["UserID"]."', '".$customer."', '".date('H:i')."', 'ปกติ', 
		'รายวัน', '".$datepicker."', '".$endate."', 

		'".$codecustomer."', '".$moneydown."', '".$moneydata."', '".$pasy."', '".$cod."', '".$moneycontact."',
		'".$percent."', '".$computer."', '".$major."', 'เป็นหนี้', 'ปกติ', '".$computer2."', '".$priceorder."', 
		'".$appleid."', '".$password."', 'ปกติ', '".$bank."', '".$pasycal."', '".$qrdata."', '".$contactstatus."', '".$daypayment."'  
		)"; 
		$objQuery = mysqli_query($objCon, $strSQL);
		...
		$objQuery = mysqli_query($objCon, $strSQL);	
		 ...
		 echo("<script>window.location = 'check_contact_onoff.php';</script>");
